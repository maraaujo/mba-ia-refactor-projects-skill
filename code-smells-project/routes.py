...
def register_routes(app):
    ...
    # Endpoint administrativo protegido por token (ver middleware.admin_required).
    app.add_url_rule("/admin/reset-db", "reset_database", admin_controller.reset_database, methods=["POST"])

    # Mantido por compatibilidade de contrato (rota original preservada), porém
    # neutralizado: nunca executa SQL vindo do cliente. Ver finding C2.
    app.add_url_rule(
        "/admin/query", "executar_query_desabilitado", admin_controller.executar_query_desabilitado, methods=["POST"]
    )
